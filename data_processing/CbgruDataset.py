...
class CbgruDataset(Dataset):
    def __init__(self, word2vec_dir, fastText_dir, labels_path, name_path, preload=True):
        labels = []
    ...
    def load_item(self, idx):
        ...
        try:
            ...
            # label is loaded in __getitem__ or cached separately, 
            # but to be consistent with cache, we can return tuple
            return word2vec, fastText
    ...
    def __getitem__(self, idx):
        if self.preload:
            word2vec, fastText = self.data_cache[idx]
        else:
            word2vec, fastText = self.load_item(idx)
