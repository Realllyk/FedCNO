...
from data_processing.LgvMandoDataset import LgvMandoDataset
...
from data_processing.preprocessing import get_cbgru_feature, relabel_with_pretrained_knn, read_pretrain_feature, reduced_name_labels, gen_noise_labels, gen_sys_noise_labels, get_graph_feature, get_pattern_feature, gen_sys_noise_labels_kmeans, compute_global_clusters, resolve_pretrain_feature_dir
# from data_processing.cbgru_dataset import CbgruDataset


def _ensure_mando_vul(vul):
    if vul != "tod":
        raise ValueError("MANDO in this project currently supports only vul='tod'.")
...
def gen_client_ds(model_type, client_id, vul, noise_type, noise_rate, num_neigh, assigned_clusters=None, global_cluster_map=None, n_clusters=20, seed=42, data_dir=None):
    ...
    if model_type == 'CBGRU':
        ...
        ds = gen_mando_client_ds(client_id, vul, noise_type, noise_rate, num_neigh, assigned_clusters, global_cluster_map, n_clusters, seed, data_dir)
    return ds
...
def gen_test_dl(model_type, vul, data_dir=None):
    ...
    if model_type == "CBGRU":
        ...
        ds = gen_mando_test_ds(vul, data_dir=data_dir)
        return DataLoader(ds, pin_memory=True, collate_fn=mando_collate_fn)
...
def gen_mando_client_ds(client_id, vul, noise_type, noise_rate, num_neigh, assigned_clusters=None, global_cluster_map=None, n_clusters=20, seed=42, data_dir=None):
    if data_dir is None:
        raise ValueError("data_dir must be provided")
    _ensure_mando_vul(vul)
    graph_dir = os.path.join(data_dir, f"mando_graph/{vul}")
    ...
    if noise_type == 'non_noise' or noise_type == 'fn_noise':
        ...
        cluster_indices = assigned_clusters[client_id] if assigned_clusters else None
        noise_labels = gen_sys_noise_labels_kmeans(
            names_path, labels_path, pre_feature_dir, noise_rate,
            n_clusters=n_clusters, seed=seed,
            assigned_cluster_indices=cluster_indices,
            global_cluster_map=global_cluster_map
        )
    else:
        noise_labels = ds.labels
    ds.labels = noise_labels
    return ds
...
def gen_lgv_mando_ds(client_id, vul, noise_type, noise_rate, num_neigh, assigned_clusters=None, global_cluster_map=None, n_clusters=20, seed=42, predefined_labels=None, data_dir=None):
    ...
    names_path = os.path.join(client_dir, "contract_name_train.txt")
    labels_path = os.path.join(client_dir, "label_train.csv")
    ds = LgvMandoDataset(graph_dir, labels_path, names_path)

    if predefined_labels is not None:
        noise_labels = predefined_labels
    elif noise_type == 'non_noise' or noise_type == 'fn_noise':
        noise_labels = gen_noise_labels(names_path, labels_path, noise_type, noise_rate)
    elif noise_type == 'sys_noise':
        pre_feature_dir = resolve_pretrain_feature_dir(data_dir, vul)
        cluster_indices = assigned_clusters[client_id] if assigned_clusters else None
...
def gen_mando_test_ds(vul, data_dir=None):
    if data_dir is None:
        raise ValueError("data_dir must be provided")
    _ensure_mando_vul(vul)
    graph_dir = os.path.join(data_dir, f"mando_graph/{vul}")
    names_path = os.path.join(data_dir, f"graduate_client_split/mando/{vul}/contract_name_test.txt")
    labels_path = os.path.join(data_dir, f"graduate_client_split/mando/{vul}/label_test.csv")
    return MandoDataset(graph_dir, labels_path, names_path)
...
def gen_valid_ds(model_type, vul, data_dir=None):
    ...
    if model_type == "CBGRU":
        ...
        _ensure_mando_vul(vul)
        graph_dir = os.path.join(data_dir, f"mando_graph/{vul}")
        names_path = os.path.join(data_dir, f"graduate_client_split/mando/{vul}/contract_name_valid.txt")
        labels_path = os.path.join(data_dir, f"graduate_client_split/mando/{vul}/label_valid.csv")
        ds = MandoDataset(graph_dir, labels_path, names_path)
    return ds
...
def gen_valid_dl(model_type, vul, data_dir=None):
    ...
    if model_type == "MANDO":
        dl = DataLoader(ds, collate_fn=mando_collate_fn)
    else:
        dl = DataLoader(ds)
    return dl
